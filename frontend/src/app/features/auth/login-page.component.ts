  ...
  template: `
    <section class="auth-panel">
      ...
      <form [formGroup]="form" (ngSubmit)="onSubmit()" novalidate>
        ...
        <button type="submit" [disabled]="isSubmitting">{{ i18n.t('auth.login.submit') }}</button>
      </form>

      <p class="feedback error" *ngIf="errorMessage">{{ errorMessage }}</p>
      <p class="route-link">
        {{ i18n.t('nav.register') }}:
        <a routerLink="/register">{{ i18n.t('auth.register.submit') }}</a>
      </p>
    </section>
  `,
  styles: `
  ...
  onSubmit(): void {
    ...
    this.authService.login(this.form.getRawValue()).subscribe({
      next: () => {
        ...
        const target = role === 'PATIENT' ? '/patient-portal' : '/dashboard';
